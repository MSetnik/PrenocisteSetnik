	...
	function codepeople_add_promote_banner($wp_admin_bar)
	{
		...
		if(isset($_POST['codepeople_promote_banner_nonce']) && wp_verify_nonce($_POST['codepeople_promote_banner_nonce'], __FILE__))
		{
			if(
				...
				!empty($codepeople_promote_banner_plugins[sanitize_key($_POST['codepeople_promote_banner_plugin'])])
			)
			{
				set_transient( 'codepeople_promote_banner_'.sanitize_key($_POST['codepeople_promote_banner_plugin']), -1, 0);
				if(
					!empty($_POST['codepeople_promote_banner_action']) &&
					$_POST['codepeople_promote_banner_action'] == 'set-review' &&
					!empty($codepeople_promote_banner_plugins[sanitize_key($_POST['codepeople_promote_banner_plugin'])]['plugin_url'])
				)
				{
					print '<script>document.location.href="'.esc_js($codepeople_promote_banner_plugins[sanitize_key($_POST['codepeople_promote_banner_plugin'])]['plugin_url']).'";</script>';
				}
			}
		}

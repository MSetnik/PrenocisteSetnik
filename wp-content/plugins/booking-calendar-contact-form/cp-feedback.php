<?php
add_action( 'admin_enqueue_scripts', 'cpdexbccf_feedback_insert_adminScripts', 1); 
add_action( 'wp_ajax_cpdexbccf_feedback', 'cpdexbccf_feedback' );
$cpdexbccf_plugfile =  'dex_bccf.php';
$cpdexbccf_plugslug = 'booking-calendar-contact-form';
$cpdexbccf_supportlink = 'https://bccf.dwbooster.com/contact-us';
$cpdexbccf_supportlink_full = $cpdexbccf_supportlink . '?priority-support=yes';

if (!get_option('installed_'.$cpdexbccf_plugslug, ''))
    update_option('installed_'.$cpdexbccf_plugslug, time() );

function cpdexbccf_feedback_insert_adminScripts($hook) {
    if( 'plugins.php' == $hook  )            
    {
        wp_enqueue_style('wp-jquery-ui-dialog');
        wp_enqueue_script('jquery');
        wp_enqueue_script('jquery-ui-dialog');       
        add_action( 'admin_footer', 'cpdexbccf_feedback_javascript' );            
    }
}
...
function cpdexbccf_feedback() {
    global $cpdexbccf_plugfile, $cpdexbccf_plugslug;
    $plugin_data = get_plugin_data( dirname(__FILE__).'/'.$cpdexbccf_plugfile );
    $plugin_version = $plugin_data['Version'];
    $time = time() - get_option('installed_'.$cpdexbccf_plugslug, '');
    $data = array(
...
function cpdexbccf_feedback_javascript() { 
    global $cpdexbccf_plugslug, $cpdexbccf_supportlink, $cpdexbccf_supportlink_full; 
      ?>
	<script type="text/javascript">

$ = jQuery.noConflict()
$( window ).load(function() {
        document.querySelector('[data-slug="<?php echo $cpdexbccf_plugslug; ?>"] .deactivate a').addEventListener('click', function(event){
            event.preventDefault()
            var urlRedirect = document.querySelector('[data-slug="<?php echo $cpdexbccf_plugslug; ?>"] .deactivate a').getAttribute('href');
            ...
            $('<div title="QUICK FEEDBACK"><div style="padding:10px;">'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpdexbccf_update_reason(this);" value="no-work"> The plugin didn\'t work<br />'+
                '<div id="abcm_nowork" style="margin-left:25px;display:none;padding:10px;border:1px dotted gray;color:#660000"><strong>We can help!</strong> We offer <strong>free support</strong> for this plugin. Feel free to open a support ticket at <a href="<?php echo $cpdexbccf_supportlink_full; ?>"><strong><?php echo $cpdexbccf_supportlink; ?></strong></a> and we will be happy to help.</div></div>'+
                
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpdexbccf_update_reason(this);" value="-"> I don\'t like to share my information with you<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpdexbccf_update_reason(this);" value="temporary-deactivation"> It\'s a temporary deactivation. I\'m just debugging an issue.<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpdexbccf_update_reason(this);" value="better-plugin"> I found a better plugin<br />'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpdexbccf_update_reason(this);" value="other"> Other<br />'+
      		   ...
      		   '</div></div>'
                  ...
                  buttons: [
                      ...
                      },
                      {
                        ...
                        click: function() {
                                   window.open('<?php echo $cpdexbccf_supportlink_full; ?>');
      						       $(this).dialog("close");
      				           }                            
                      },
                      {
...
function cpdexbccf_update_reason(field)
{

...
function _dex_bccf_install() {
    ...
    $sql = "CREATE TABLE `".$wpdb->prefix.DEX_BCCF_CONFIG_TABLE_NAME."` (".
                   ...
                   "`".TDE_BCCFCONFIG_USER."` varchar(200) default NULL,".
                   "`".TDE_BCCFCONFIG_PASS."` varchar(200) default NULL,".
                   ...
                   "`calendar_language` varchar(20) DEFAULT '' NOT NULL,".
                   "`calendar_mode` varchar(20) DEFAULT '' NOT NULL,".
                   ...
                   "`url_ok` text,".
                   "`url_cancel` text,".
                   "`paypal_language` varchar(20) DEFAULT '' NOT NULL,".
                   // copy to user
                   ...
                   "`vs_text_is_required` VARCHAR(200) DEFAULT '' NOT NULL,".
...
function dex_bccf_check_posted_data()
{
    ...
    if (isset( $_GET['dex_bccf'] ) &&  $_GET['dex_bccf'] == 'bccf_loadmindate' && is_admin() )
    {
        if ($_GET["code"] == '')
            echo '';
        else
    ...
    if (isset( $_GET['dex_bccf'] ) &&  $_GET['dex_bccf'] == 'bccf_loadmaxdate' && is_admin() )
    {
        if ($_GET["code"] == '')
            echo '';
        else
...
<input type="hidden" name="business" value="<?php echo trim(dex_bccf_get_option('paypal_email', dex_bccf_get_default_paypal_email())); ?>" />
...
<input type="hidden" name="return" value="<?php echo esc_url(trim(dex_bccf_get_option('url_ok', DEX_BCCF_DEFAULT_OK_URL))); ?>">
<input type="hidden" name="cancel_return" value="<?php echo esc_url(dex_bccf_get_option('url_cancel', DEX_BCCF_DEFAULT_CANCEL_URL)); ?>" />
<input type="hidden" name="currency_code" value="<?php echo dex_bccf_clean_currency(dex_bccf_get_option('currency', DEX_BCCF_DEFAULT_CURRENCY)); ?>" />
...
function dex_bccf_save_options()
{
    ...
    $data = array(
         ...
         'calendar_fixedmode' => sanitize_text_field(@$_POST["calendar_fixedmode"]?"1":"0"),         
         ...
         'enable_paypal_option_yes' => sanitize_text_field(@$_POST['enable_paypal_option_yes']?$_POST['enable_paypal_option_yes']:DEX_BCCF_DEFAULT_PAYPAL_OPTION_YES),
         'enable_paypal_option_no' => sanitize_text_field(@$_POST['enable_paypal_option_no']?$_POST['enable_paypal_option_no']:DEX_BCCF_DEFAULT_PAYPAL_OPTION_NO),
         ...
         'cp_cal_checkboxes' => sanitize_textarea_field(@$_POST["cp_cal_checkboxes"]),
         'cp_cal_checkboxes_type' => sanitize_text_field(@$_POST["cp_cal_checkboxes_type"])
	);
...
function dex_bccf_data_management_loaded() 
{
    ...
    $action = @$_POST['dex_bccf_do_action_loaded'];
...
function dex_bccf_translate_json($str)
{
    ...
    {
        $form_data[0][$i]->title = __($form_data[0][$i]->title,'booking-calendar-contact-form');   
        $form_data[0][$i]->userhelpTooltip = __($form_data[0][$i]->userhelpTooltip,'booking-calendar-contact-form'); 

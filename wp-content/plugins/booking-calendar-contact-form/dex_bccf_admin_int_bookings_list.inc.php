...
if (wp_verify_nonce( $_REQUEST['_wpnonce'], 'uname_bccf' ) && isset($_GET['delmark']) && $_GET['delmark'] != '')
{
...
else if (wp_verify_nonce( $_REQUEST['_wpnonce'], 'uname_bccf' ) && isset($_GET['del']) && $_GET['del'] == 'all')
{    
...
else if (wp_verify_nonce( $_REQUEST['_wpnonce'], 'uname_bccf' ) && isset($_GET['ld']) && $_GET['ld'] != '')
{
...
<script type="text/javascript">
 function cp_deleteMessageItem(id)
 {
    ...
    {        
        document.location = 'admin.php?page=dex_bccf.php&cal=<?php echo intval($_GET["cal"]); ?>&list=1&_wpnonce=<?php echo $nonce_un; ?>&ld='+id+'&r='+Math.random();
    }
 }
 ...
 function cp_deleteall()
 {
    ...
    {        
        document.location = 'admin.php?page=dex_bccf.php&cal=<?php echo intval(CP_BCCF_CALENDAR_ID); ?>&list=1&del=all&_wpnonce=<?php echo $nonce_un; ?>';
    }    
 }
 function cp_markall()
 {
...
<form action="admin.php" method="get">
 ...
 Search for: <input type="text" name="search" value="<?php echo esc_attr(sanitize_text_field($_GET["search"])); ?>" /> &nbsp; &nbsp; &nbsp; 
 From: <input autocomplete="off" type="text" id="dfrom" name="dfrom" value="<?php echo esc_attr(sanitize_text_field($_GET["dfrom"])); ?>" /> &nbsp; &nbsp; &nbsp; 
 To: <input autocomplete="off" type="text" id="dto" name="dto" value="<?php echo esc_attr(sanitize_text_field($_GET["dto"])); ?>" /> &nbsp; &nbsp; &nbsp; 
...
echo paginate_links(  array(
    'base'         => 'admin.php?page=dex_bccf.php&cal='.CP_BCCF_CALENDAR_ID.'&list=1%_%&dfrom='.urlencode(sanitize_text_field($_GET["dfrom"])).'&dto='.urlencode(sanitize_text_field($_GET["dto"])).'&search='.urlencode(sanitize_text_field($_GET["search"])),
    'format'       => '&p=%#%',
    'total'        => $total_pages,
...
<form name="dex_table_form" id="dex_table_form" action="admin.php" method="get">
 <input type="hidden" name="page" value="dex_bccf.php" />
 <input type="hidden" name="_wpnonce" value="<?php echo $nonce_un; ?>" />
